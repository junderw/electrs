...
pub(super) mod sigops {
    ...
    fn count_sigops(script: &script::Script, accurate: bool) -> Result<usize, script::Error> {
        let mut n = 0;
    ...
    fn get_legacy_sigop_count(tx: &Transaction) -> Result<usize, script::Error> {
        let mut n = 0;
    ...
    fn get_p2sh_sigop_count(
        ...
        for (input, prevout) in tx.input.iter().zip(previous_outputs.iter()) {
            if prevout.script_pubkey.is_p2sh() {
                ...
                {
                    let script = script::Script::from_byte_iter(redeem.iter().map(|v| Ok(*v)))
    ...
    fn sig_ops(witness: &Witness, witness_version: u8, witness_program: &[u8]) -> usize {
        ...
        match (witness_version, witness_program.len()) {
            (0, 20) => 1,
            (0, 32) => {
                if let Some(Ok(n)) = last_witness
                    .map(|sl| sl.iter().map(|v| Ok(*v)))
                    .map(script::Script::from_byte_iter)
                    .map(|s| count_sigops(&s.map_err(|_| script::Error::EarlyEndOfScript)?, true))
                {
                    n
                } else {
                    0
